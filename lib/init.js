(function(ei, $) {
    ...
    ei.elroi.helpers = {
        ...
        getDataValues : function(allSeries, seriesOptions) {
            ...
            if(!ei.elroi.helpers.hasData(allSeries)){
                return [[0]];
            }
        ...
        maxValues : function(dataValuesSet, seriesOptions, graph) {
            ...
            function distortMaxValuesBy() {
                ...
                // Point flags
                var hasPointFlags = ei.elroi.helpers.hasPointFlags(graph.allSeries);
     ...
     */
    ei.elroi.init = function(graph) {

        var seriesOptions = ei.elroi.helpers.seriesOptions(graph.allSeries, graph.options.seriesDefaults),
            maxVals = [],
            minVals = [],
            dataValuesSet = ei.elroi.helpers.getDataValues(graph.allSeries, seriesOptions),
            sums = ei.elroi.helpers.sumSeries(dataValuesSet),
            hasData = ei.elroi.helpers.hasData(graph.allSeries);
        ...
        maxVals = ei.elroi.helpers.maxValues(dataValuesSet, seriesOptions, graph);
        minVals = ei.elroi.helpers.minValues(dataValuesSet, seriesOptions);
