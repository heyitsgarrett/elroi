(function($) {
    ...
    elroi.helpers = {
        ...
        getDataValues : function(allSeries, seriesOptions) {
            ...
            if(!elroi.helpers.hasData(allSeries)){
                return [[0]];
            }
        ...
        maxValues : function(dataValuesSet, seriesOptions, graph) {
            ...
            function distortMaxValuesBy() {
                ...
                // Point flags
                var hasPointFlags = elroi.helpers.hasPointFlags(graph.allSeries);
     ...
     */
    elroi.init = function(graph) {

        var seriesOptions = elroi.helpers.seriesOptions(graph.allSeries, graph.options.seriesDefaults),
            maxVals = [],
            minVals = [],
            dataValuesSet = elroi.helpers.getDataValues(graph.allSeries, seriesOptions),
            sums = elroi.helpers.sumSeries(dataValuesSet),
            hasData = elroi.helpers.hasData(graph.allSeries);
        ...
        maxVals = elroi.helpers.maxValues(dataValuesSet, seriesOptions, graph);
        minVals = elroi.helpers.minValues(dataValuesSet, seriesOptions);
